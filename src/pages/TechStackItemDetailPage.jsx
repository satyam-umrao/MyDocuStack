...
const isVisualUIItem = (stack, item) => {
  ...
  if (stack.id === 'nextjs') {
    ...
    return name.includes('page') || name.includes('layout') || name.includes('image') || 
           name.includes('link') || name.includes('loading') || name.includes('error') || 
           name.includes('not-found') || name.includes('client');
  }
  
  // 5. JavaScript DOM and interactive events
  if (stack.id === 'js') {
    const name = (item.name || '').toLowerCase();
    return name.includes('addeventlistener') || name.includes('localstorage') || 
           name.includes('sessionstorage') || name.includes('timeout') || 
           name.includes('interval') || name.includes('fetch');
  }
...
const getExampleLanguage = (stackId, item, ex) => {
  ...
  switch (stackId) {
    ...
    case 'nestjs': return 'typescript';
    case 'java':
    case 'springboot': return 'java';
    case 'golang':
    case 'gin':
    case 'gorm': return 'go';
    ...
    case 'aws':
      return (ex?.code?.trim().startsWith('aws ') || ex?.code?.trim().startsWith('#')) ? 'bash' : 'typescript';
    default:
      return 'javascript';
  }
};
...
export default function TechStackItemDetailPage({ item, stack, onBackToStack, onBackToMaster }) {
  ...
  const runSimulation = () => {
    ...
    setTimeout(() => {
      ...
      if (item.category === 'Semantic' || item.category === 'Forms' || item.category === 'Interactive') {
        setSimulatorOutput(`[DOM Engine] Rendered <${item.name.replace(/<|>/g, '').split(',')[0].trim()}> successfully.\n✓ Accessibility tree built (Role: landmark, WCAG AAA compliant).\n✓ Layout shift: 0.00 CLS | Paint time: 1.2ms.`);
      } else if (item.category === 'Layout' || item.category === 'Responsive' || item.category === 'Styling & Motion') {
        setSimulatorOutput(`[CSS Layout Engine] Computed styles for ${item.name}:\n✓ Display box model initialized.\n✓ GPU compositor layer created (60 fps frame budget maintained).\n✓ Reflow cost: 0.14ms.`);
      } else if (item.category === 'Asynchronous' || item.category === 'Runtime Architecture') {
        setSimulatorOutput(`[V8 Engine] Microtask resolved in ${Math.floor(Math.random() * 8 + 4)}ms.\n✓ Call stack emptied -> Microtask queue flushed -> Event loop tick complete.`);
      } else if (item.category === 'Type System' || item.category === 'Type Transformations') {
        setSimulatorOutput(`[TypeScript Compiler] Type check passed for ${item.name}.\n✓ Zero type errors found across definitions.\n✓ Emission: Target ES2024 with zero runtime overhead.`);
      } else if (item.category === 'Hooks & State' || item.category === 'Lifecycle & Sync' || item.category === 'Global State') {
        setSimulatorOutput(`[React Fiber] Re-rendered component with updated state (Render #${simCounter}).\n✓ Reconciliation diff: 1 node updated.\n✓ Commit phase duration: 0.38ms.`);
      } else if (item.category === 'Architecture' || item.category === 'Data Mutation') {
        setSimulatorOutput(`[Next.js Server] Executed Server Component / Action.\n✓ revalidatePath() cache purged.\n✓ Server-timing: 24.1ms.`);
      } else if (stack.id === 'docker') {
        setSimulatorOutput(`$ ${item.methods?.[0]?.name || 'docker run'} --rm test-instance\n✓ Container started: sha256:4a8b9c20f\n✓ Health check: healthy (0 restart)\n✓ Process isolated in user namespace.`);
      } else if (stack.id === 'kubernetes') {
        setSimulatorOutput(`$ kubectl apply -f spec.yaml\n${item.name.toLowerCase()} configured.\n✓ Status: 3/3 Pods Ready | 0 Restarts\n✓ Target endpoints updated in kube-dns.`);
      } else if (stack.id === 'aws') {
        setSimulatorOutput(`$ aws ${item.name.toLowerCase().replace(/[^a-z0-9]/g, '-')} status\n{\n  "Status": "ACTIVE",\n  "Latency": "18.4ms",\n  "Region": "us-east-1",\n  "ExecutionId": "exec-${Math.floor(Math.random() * 90000 + 10000)}"\n}`);
      } else if (stack.id === 'postgresql') {
        setSimulatorOutput(`EXPLAIN (ANALYZE, BUFFERS) Query Plan:\n-> Index Scan on table (cost=0.15..8.24 rows=1 width=64)\n   Buffers: shared hit=4\nExecution Time: 0.052 ms (Hot Cache Hit)`);
      } else {
        setSimulatorOutput(`✓ ${item.name} executed successfully in ${stack.name} runtime.\n✓ Status: 200 OK | Latency: 1.2ms\n✓ Memory impact: Minimal (Safe heap allocation).`);
      }
    }, 450);
  };
    ...
    <div className="detail-page-wrapper" style={{ paddingBottom: '80px' }}>
      <div className="container" style={{ maxWidth: '1000px' }}>
        ...
        <div className="breadcrumb-apple">
          <a href="#" onClick={(e) => { e.preventDefault(); onBackToMaster && onBackToMaster(); }}>
            My DocuStack
          </a>
          <span>/</span>
          <a href="#" onClick={(e) => { e.preventDefault(); onBackToStack && onBackToStack(); }}>
            {stack.name}
          </a>
          <span>/</span>
          <span style={{ color: 'var(--color-corona-muted)' }}>{item.category}</span>
          <span>/</span>
          <span style={{ color: stack.accentColor || 'var(--color-corona-blue)', fontWeight: '600' }}>
            {item.name}
          </span>
        </div>
        ...
        <div 
          ...
          style={{ borderTop: `3px solid ${stack.accentColor || 'var(--color-corona-green)'}` }}
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flexWrap: 'wrap' }}>
              <span 
                ...
                style={{ 
                  borderColor: stack.accentColor ? `${stack.accentColor}66` : undefined,
              ...
              <span 
                className="filter-chip"
                style={{ background: 'rgba(255, 255, 255, 0.08)', cursor: 'default', color: '#ffffff' }}
              >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          {item.demo && (
            <div className="block-card-apple" style={{ margin: 0, backgroundColor: 'var(--color-corona-surface)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
                ...
                >
                  {isRunning ? <RefreshCw size={13} className="spin-animate" /> : <Play size={13} />}
              ...
              >
                <div style={{ color: 'var(--color-corona-muted)', fontSize: '11px', marginBottom: '4px' }}>
                  $ {stack.id === 'docker' ? 'docker daemon --status' : stack.id === 'kubernetes' ? 'kubectl cluster-info' : stack.id === 'postgresql' ? 'psql -U postgres -d enterprise' : 'runtime-console --inspect'}
                </div>
                {simulatorOutput || `> Ready to execute ${item.name} in ${stack.name} runtime. Click "Run Simulation" above to test.`}
              </div>
            </div>
          )}
                ...
                <table className="props-table">
                  ...
                  <tbody>
                    {item.props.map((prop, idx) => (
                      <tr key={idx}>
                        <td className="prop-name" style={{ color: stack.accentColor || 'var(--color-corona-blue)' }}>
                          {prop.name}
                        </td>
                ...
                <table className="props-table">
                  ...
                  <tbody>
                    {item.methods.map((method, idx) => (
                      <tr key={idx}>
                        <td className="prop-name" style={{ color: 'var(--color-corona-green)' }}>
                          {method.name}
                        </td>
          ...
          {item.codeExamples && item.codeExamples.length > 0 && (
            <div className="block-card-apple" style={{ margin: 0 }}>
              <h3 className="block-title-apple" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <Code size={20} style={{ color: stack.accentColor || 'var(--color-corona-blue)' }} />
